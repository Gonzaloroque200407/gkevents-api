...
const pool = mysql.createPool({
  host: process.env.DB_HOST,      // ex.: 192.168.249.130
  port: process.env.DB_PORT || 3306,
  user: process.env.DB_USER,      // ex.: user_remote
  password: process.env.DB_PASS,  // ex.: 123456
  database: process.env.DB_NAME,  // ex.: gkevents
...
const sessionStore = new MySQLStore(
  ...
  // usa um pool individual do express-mysql-session
  mysql.createPool({
...
// ---------- LOGIN / CADASTRO ----------
// Nota: Senha armazenada como HEX de SHA2(…,256) em minúsculo (compatível com o que você já tem)
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    // compara no MySQL para bater com o formato existente
    const sql =
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    const sql =
      'INSERT INTO users (name,email,password_hash,role) VALUES (?,?,LOWER(SHA2(?,256)),?)';
    const [r] = await pool.query(sql, [name || null, email.trim(), password, 'user']);

    res.json({
      ok: true,
      user: { id: r.insertId, name: name || null, email: email.trim(), role: 'user' }
    });
  } catch (e) {
  ...
  try {
    ...
    const [attendees] = await pool.query(
      `SELECT ea.user_id, u.name, u.email
        ...
        WHERE ea.event_id=? ORDER BY u.name`,
      [id]
    );
...
app.put('/api/events/:id', async (req, res) => {
  ...
  try {
    ...
    await pool.query('UPDATE events SET name=?, date=?, location=? WHERE id=?', [
      name,
      date,
      location,
      id
...
app.delete('/api/events/:id', async (req, res) => {
  ...
  try {
    const id = Number(req.params.id);
    if (!id) return res.status(400).json({ ok: false, error: 'bad_event_id' });
...
// ---------- CONFIRMAR / CANCELAR PRESENÇA (CORRIGIDO) ----------
app.post('/api/events/:id/confirm', async (req, res) => {
  const user = authUser(req, res);
  if (!user) return;

  const eventId = Number(req.params.id);
  if (!eventId) return res.status(400).json({ ok: false, error: 'bad_event_id' });

  try {
    // evita erro em caso de dupla confirmação
    const sql =
      'INSERT INTO event_attendees (event_id, user_id) VALUES (?, ?) ' +
      'ON DUPLICATE KEY UPDATE event_id = VALUES(event_id)';
    await pool.query(sql, [eventId, user.id]);
...
app.delete('/api/events/:id/confirm', async (req, res) => {
  ...
  try {
    await pool.query('DELETE FROM event_attendees WHERE event_id=? AND user_id=?', [
      eventId,
      user.id
    ]);
